...
namespace ff {
	...
	void Renderer::projectObject(const Object3D::Ptr& object, uint32_t groupOrder, bool sortObjects) noexcept {
		...
		if (object->m_isGroup) {
			...
			groupOrder = group->m_groupOrder;
		}
		else if (object->m_isLight) {
		...
		//如果是可渲染物体
		else if (object->m_isRenderableObject) {
			//骨骼
			if (object->m_isSkinnedMesh) {
	...
	void Renderer::renderObjects(
		...
		const auto overrideMaterial = scene->m_isScene ? scene->m_overrideMaterial : nullptr;

		for (const auto& renderItem : renderItems) {
			const auto object = renderItem->m_object;
			const auto geometry = renderItem->m_geometry;
			const auto material = overrideMaterial == nullptr ? renderItem->m_material : overrideMaterial;
	...
	void Renderer::renderBufferDirect(
		...
		//draw
		if (index) {
			glDrawElements(toGL(material->m_drawMode), index->getCount(), toGL(index->getDataType()), 0);
		}
		else {
			glDrawArrays(toGL(material->m_drawMode), 0, position->getCount());
		}
	...
	DriverProgram::Ptr Renderer::setProgram(
		...
		if (material->m_version == dMaterial->mVersion) {
			...
			if (material->m_diffuseMap != dMaterial->mDiffuseMap) {
				if (material->m_diffuseMap == nullptr || dMaterial->mDiffuseMap == nullptr) {
					needsProgramChange = true;
				}
			}

			if (material->m_envMap != dMaterial->mEnvMap) {
				if (material->m_envMap == nullptr || dMaterial->mEnvMap == nullptr) {
					needsProgramChange = true;
				}
			}

			if (material->m_normalMap != dMaterial->mNormalMap) {
				if (material->m_normalMap == nullptr || dMaterial->mNormalMap == nullptr) {
					needsProgramChange = true;
				}
			}

			if (material->m_specularMap != dMaterial->mSpecularMap) {
				if (material->m_specularMap == nullptr || dMaterial->mSpecularMap == nullptr) {
					needsProgramChange = true;
				}
			}
			...
			if (object->m_isSkinnedMesh != dMaterial->mSkinning) {
				dMaterial->mSkinning = object->m_isSkinnedMesh;
				needsProgramChange = true;
			}
			...
			needsProgramChange = true;
			dMaterial->mVersion = material->m_version;
		}
		...
		const DriverProgram::Parameters::Ptr& parameters) noexcept
	{
		...
		dMaterial->mDiffuseMap = material->m_diffuseMap;
		dMaterial->mEnvMap = material->m_envMap;
		dMaterial->mNormalMap = material->m_normalMap;
		dMaterial->mSpecularMap = material->m_specularMap;
	}
	...
	void Renderer::setRenderTarget(const RenderTarget::Ptr& renderTarget) noexcept {
		...
		mState->bindFrameBuffer(dRenderTarget->m_frameBuffer);
	}
