...
//--------driver programs----------------------------
	...
	DriverProgram::Parameters::Ptr DriverPrograms::getParameters(
		...
		//mDirectionalLightCount = lights->mState.mDirectionalCount;
		...
		if (material->m_diffuseMap != nullptr) {
			parameters->mHasDiffuseMap = true;
		}

		if (material->m_envMap != nullptr) {
		...
		if (object->m_isRenderableObject) {
		...
		if (material->m_isDepthMaterial) {
			auto depthMaterial = std::static_pointer_cast<DepthMaterial>(material);
			parameters->mDepthPacking = depthMaterial->m_packing;
		}
