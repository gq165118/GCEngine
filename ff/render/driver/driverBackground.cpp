...
namespace ff {
	...
	void DriverBackground::render(const DriverRenderList::Ptr& renderList, const Scene::Ptr& scene) {
		...
		auto background = scene->m_background;
		...
		if (mBoxMesh == nullptr) {
			...
			material->m_side = Side::BackSide;
			material->m_envMap = background;
			...
			//lamda表达式大家要自行学习
			mBoxMesh->m_onBeforeRenderCallback = [&](Renderer* render, Scene* scene, Camera* camera) {
