...
namespace ff {
	...
	void DriverState::setMaterial(const Material::Ptr& material) noexcept {
		...
		if (mCurrentRaster.mSide != material->m_side) {
			if (material->m_side == Side::DoubleSide) {
				gl::disable(GL_CULL_FACE);
			}
			else {
				gl::enable(GL_CULL_FACE);
				glCullFace(toGL(material->m_side));
			}
			mCurrentRaster.mSide = material->m_side;
		}

		if (mCurrentRaster.mFrontFace != material->m_frontFace) {
			mCurrentRaster.mFrontFace = material->m_frontFace;
			glFrontFace(toGL(material->m_frontFace));
		}

		setBlending(
			material->m_blendingType,
			material->m_transparent,
			material->m_blendSrc,
			material->m_blendDst,
			material->m_blendEuqation,
			material->m_blendSrcAlpha,
			material->m_blendDstAlpha,
			material->m_blendEquationAlpha
		);

		setDepth(material->m_depthTest, material->m_depthWrite, material->m_depthFunction, material->m_depthClearColor);
